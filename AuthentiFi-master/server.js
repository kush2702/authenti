...
const address = '0x67B7ae9CB390dd2B0A3439D4dbD5B10A6aD1925C';

var contract = new web3.eth.Contract(abiArray,address);

web3.eth.defaultAccount = '0xa90472499700e0B476B6D9d49C72d10746d8E056';
const Account='';
...
function createCustomer(hashedEmail, name, phone) {
    return contract.methods.createCustomer(hashedEmail, name, phone).call({from: web3.eth.defaultAccount, gas: 3000000 });
}
...
function createRetailer(retailerHashedEmail, retailerName, retailerLocation) {
    return contract.methods.createRetailer(retailerHashedEmail, retailerName, retailerLocation).call({from: web3.eth.defaultAccount, gas: 3000000 });
}
...
app.post('/addRetailerToCode', (req, res) => {
    ...
    let ok = contract.methods.addRetailerToCode(code, hashedEmail);
    if(!ok) {
        return res.status(400).send('Error');
    }
...
app.post('/myAssets', (req, res) => {
    ...
    let arrayOfCodes = contract.methods.getCodes(hashedEmail);
    console.log(`Email ${email}`);
    console.log(`Customer has these product codes: ${arrayOfCodes} \n`);
    for (code in arrayOfCodes) {
        let ownedCodeDetails = contract.methods.getOwnedCodeDetails(arrayOfCodes[code]);
        let notOwnedCodeDetails = contract.methods.getNotOwnedCodeDetails(arrayOfCodes[code]);
        myAssetsArray.push({
...
app.post('/stolen', (req, res) => {
    ...
    let hashedEmail = hashMD5(email);
    console.log(`Email: ${email} \n`);
    let ok = contract.methods.reportStolen(code, hashedEmail);
    if (!ok) {
...
app.post('/getProductDetails', (req, res) => {
    ...
    for (let i = 0; i < QRCodes.length; i++) {
        if (QRCode === QRCodes[i]['QRCode']) {
            ...
            // QR Codes are valid only for 600 secs
            if (timeElapsed <= 600) {
                let ownedCodeDetails = contract.methods.getOwnedCodeDetails(code);
                let notOwnedCodeDetails = contract.methods.getNotOwnedCodeDetails(code);
            ...
            if (timeElapsed <= 600) {
                if(QRCodes[i]['confirm'] === '1'){
                    ...
                    if(QRCodes[i]['retailer'] === '1'){
                        ...
                        ok = contract.methods.initialOwner(code, hashedSellerEmail, hashedBuyerEmail).call({from: web3.eth.defaultAccount, gas: 3000000 });
                    } else {
                        console.log('Performing transaction for customer\n');
                        ok = contract.methods.changeOwner(code, hashedSellerEmail, hashedBuyerEmail).call({ from: web3.eth.defaultAccount, gas: 3000000 });
                    }
                    if (!ok) {
                        return res.status(400).send('Error');
                    }
...
function initialOwner(code, retailerHashedEmail, customerHashedEmail) {
    return contract.methods.initialOwner(code, retailerHashedEmail, customerHashedEmail).call({ from: web3.eth.defaultAccount, gas: 3000000 });
}

// Function that creates transfers ownership of a product
function changeOwner(code, oldOwnerHashedEmail, newOwnerHashedEmail) {
    return contract.methods.changeOwner(code, oldOwnerHashedEmail, newOwnerHashedEmail).call({ from: web3.eth.defaultAccount, gas: 3000000 });
}
...
app.post('/scan', (req, res) => {
    ...
    let code = req.body.code;
    let productDetails = contract.methods.getNotOwnedCodeDetails(code);
...
app.post('/QRCodeForManufacturer', (req, res) => {
    ...
    let ok = contract.methods.createCode(code, brand, model, status, description, manufacturerName, manufacturerLocation,
                                        manufacturerTimestamp).call( { from: web3.eth.defaultAccount, gas: 3000000 });
...
app.get('/getCustomerDetails', (req, res) => {
    ...
    let customerDetails = contract.methods.getCustomerDetails(hashedEmail);
    console.log(`Email: ${email} \n`);
