...
function createCustomer(hashedEmail, name, phone) {
    return contract.methods.createCustomer(hashedEmail, name, phone, { from: web3.eth.accounts[0], gas: 3000000 });
}
...
function createRetailer(retailerHashedEmail, retailerName, retailerLocation) {
    return contract.methods.createRetailer(retailerHashedEmail, retailerName, retailerLocation,
            ...
            if (timeElapsed <= 600) {
                if(QRCodes[i]['confirm'] === '1'){
                    ...
                    if(QRCodes[i]['retailer'] === '1'){
                        ...
                        ok = contract.methods.initialOwner(code, hashedSellerEmail, hashedBuyerEmail,
                                                        { from: web3.eth.accounts[0], gas: 3000000 });
                    } else {
                        console.log('Performing transaction for customer\n');
                        ok = contract.methods.changeOwner(code, hashedSellerEmail, hashedBuyerEmail,
                                                        { from: web3.eth.accounts[0], gas: 3000000 });
                    }
                    if (!ok) {
                        return res.status(400).send('Error');
...
function initialOwner(code, retailerHashedEmail, customerHashedEmail) {
    return contract.methods.initialOwner(code, retailerHashedEmail, customerHashedEmail,
                                        { from: web3.eth.accounts[0], gas: 3000000 });
}

// Function that creates transfers ownership of a product
function changeOwner(code, oldOwnerHashedEmail, newOwnerHashedEmail) {
    return contract.methods.changeOwner(code, oldOwnerHashedEmail, newOwnerHashedEmail,
                                        { from: web3.eth.accounts[0], gas: 3000000 });
}
